...
function getAllSheetData() {
  try {
    ...
    // 1. ดึง Master Index จากชีท Aging_Order
    ...
    // 3. จัดกลุ่มข้อมูลลงในแต่ละชีท โดยเรียงลำดับตามชีท Aging
    const allCombined = [];
    mainSheets.forEach(name => {
...
function migrateToAgingOrder() {
  ...
  if (newData.length > 0) {
    ...
    newAging.getRange(3, 2, newData.length, 1).setNumberFormat('@');
  }
